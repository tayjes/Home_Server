...
Opens a raw socket (AF_PACKET)

Broadcasts ARP requests across local subnet

Collects ARP replies

Extracts MAC address

Matches first 3 bytes (OUI) against helper/MAC.txt

Returns results to Python as a list of dictionaries
...
scan.cpython-312-x86_64-linux-gnu.so
...
{
    "ip": "192.168.1.5",
    "mac": "5c:22:da:3f:3b:f7",
    "company": "Some Vendor Name"
}
⚠ Important Notes
...
Contains IEEE OUI vendor assignments used to map MAC prefixes to company names.

📄 command.txt

Contains development commands and notes related to the project.
...
Below are the planned development phases:
...
A private, secure file-sharing solution for family members.
...
This aims to reduce dependency on third-party cloud platforms while maintaining convenience.
...
Optional integration for service/payment tracking
